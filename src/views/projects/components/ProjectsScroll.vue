    ...
    return {
      ...
      list : [{
        ...
        content: `
                  <b>优化用户体验</b><br>
                  自适应各种规格的设备屏幕；<br>
                  对图片高度不固定的瀑布流组件采用预加载方案；<br>
                  实现一键换肤，并对皮肤进行缓存处理；<br>
                  采取节流方案优化滚动事件；<br>
                  <b>复杂交互逻辑</b><br>
                  通过全局状态管理实现组件动态定位与切换；<br>
                  通过 mixins 实现在 Vue 中可复用的滚动视差功能；<br>
                  结合 CSS 3D 转换实现文字循环动画，并支持点击交互；<br>
                  结合 swiper.js 实现动态更新的轮播组件；<br>
                  `
                  ,
